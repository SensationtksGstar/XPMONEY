...
**Fases:** ✅ 1. Mascote viva (speech bubble + tap-to-talk no dashboard, `mascotSpeak.ts` shared brain, idle micro-bursts CSS). ✅ 1.5 Previsão de gastos (secção acima). **Pendentes:** 2. Token pass (superfícies neutras, matar glows, type scale) → 3. Navegação (fix rótulo "Contas"→"Movimentos", agrupar o sheet "Mais" por temas, um só add-CTA, TopBar limpa — o language toggle vai para Settings) → 4. Dashboard diet (~20 secções → glance de 3: herói score+mascote / resumo mês / recentes) → 5. Landing tighten (13 blocos → ~7). Problemas documentados no brief: rótulo "Contas" abre /transactions; 13 features enterradas no "Mais"; dashboard ~3000px com 2 ads.
