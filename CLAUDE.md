...
- **Visual identity refresh (June 2026):** all 12 mascot WebPs + hero + evolution sheets replaced with pro art. **Pipeline (reproducible, Python — rembg+scipy+PIL installed):** source PNG → `rembg` **`birefnet-general`** model (cleanest on these sources; u2net/isnet left artefacts) → core-solid belly: `scipy.ndimage.binary_fill_holes(alpha>170)` eroded 1px set to opaque (the cream belly is enclosed by the dark outline; this stops it going transparent→"black belly") → outer-edge de-fringe `clip((a-55)*1.7,0,255)` (kills white halo) → autocrop → fit 512×512 → WebP q90. **Evolution sheets** (`evolucoes-{voltix,penny}.webp`) are COMPOSITED from the 6 clean individuals (not re-keyed from the messy source sheet). **Voltix filename GOTCHA:** `IMPERIALVOLTIX.png`=evo-5 (Magnavoltix, muscular), `MAGNAVOLTIX.png`=evo-6 (Imperivoltix, 3-headed boss) — map by power-order. **Cache-bust:** bump `MASCOT_ASSET_VER` in `MascotCreature.tsx` (currently `'5'`) AND the hardcoded `?v=` in `LandingMascotShowcase.tsx` together on every re-cut.
- **⚠️ OPEN ISSUE (unsolved as of 2026-06-27):** the source PNGs have a baked **checkerboard fake-transparency** background (NOT real alpha). birefnet is much cleaner but STILL leaves small white remnants on a few spots where checkerboard pixels are CONNECTED to the subject and the SAME colour as the legit white belly (reported: penny-4 shoulder→waist, voltix-3 tail/wing, voltix-4 arm↔belly, voltix-6 wing↔waist). These can't be removed by colour/hole-fill/connected-component (single component, same colour as belly). **The only clean fix: source files with REAL alpha transparency** — the original `Pennini.png` had real alpha and cut perfectly. Recommend the user re-export each mascot as a transparent PNG (or remove.bg) and re-run the pipeline; OR per-pixel manual cleanup. Source folders: `…\xp mascotes\MASCULINO\PNG VOLTIX\` + `…\FEMININO\PNG PENNY\`.
