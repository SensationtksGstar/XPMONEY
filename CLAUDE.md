...
**Fases:** ✅ 1. Mascote viva (speech bubble + tap-to-talk no dashboard, `mascotSpeak.ts` shared brain, idle micro-bursts CSS). ✅ 1.5 Previsão de gastos (secção acima). ✅ **2. Token pass (SHIPPED July 2026, commits 9bde6a4→ed09e01)** — ver subsecção abaixo. **Pendentes:** 3. Navegação (fix rótulo "Contas"→"Movimentos", agrupar o sheet "Mais" por temas, um só add-CTA, TopBar limpa — o language toggle vai para Settings) → 4. Dashboard diet (~20 secções → glance de 3: herói score+mascote / resumo mês / recentes) → 5. Landing tighten (13 blocos → ~7). Problemas documentados no brief: rótulo "Contas" abre /transactions; 13 features enterradas no "Mais"; dashboard ~3000px com 2 ads.
